...
class IRCConnection(object):
    ...
    def __init__(self, server, port, nick, logfile=None, verbosity=2, needs_registration=True):
        self.server = server
        self.port = port
...
class IRCBot(IRCConnection, object):
    """\
    ...
    def is_ping(self, message):
        return re.match('^[:,\s]', message) is not None
    ...
    def respond(self, message, channel=None, nick=None, comm='.'):
        """\
        Wraps the connection object's respond() method
        """
        if comm == '@':
            self.conn.respond(message, nick=nick)
        else:
            self.conn.respond(message, channel=channel, nick=nick)

    def send(self, data, force=False):
        self.conn.send(data, force)
